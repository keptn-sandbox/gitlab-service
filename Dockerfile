...
RUN GOOS=linux go build -ldflags '-linkmode=external' $BUILDFLAGS -v -o gitlab-service
...
COPY --from=builder /src/gitlab-service/gitlab-service /gitlab-service
